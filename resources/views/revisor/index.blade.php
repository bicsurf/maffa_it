  ...
  <div class="container">
    <div class="row">
      ...
      <div class="col-12 col-md-7">
        <h1 class="text-center">{{ $announcement_to_check->title }}</h1><hr>
        <a href="{{ route('categoryShow', ['category' => $announcement_to_check->category]) }}" class="my-2 border-top pt-2 border-dark card-link shadow btn bg-primary bg-gradient text-light">{{ $announcement_to_check->category->name }}</a>
        <h5 class="card-text">Prezzo: {{ $announcement_to_check->price }} €</h5><hr>
        <h5>Descrizione :</h5>
        <p class="card-text">{{ $announcement_to_check->description }}</p>
        <hr id="hr-show">
        <h5 class="card-text custom-positioning">Contatta subito <strong>{{ $announcement_to_check->user->name ?? '' }}</strong> per avere più informazioni a riguardo</h5>
        <p id="p-date" class="card-text custom-positioning">Pubblicato il: {{ $announcement_to_check->created_at->format('d/m/Y') }}  </p>
      </div>
